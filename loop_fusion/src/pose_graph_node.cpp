...
void process()
{
    while (true)
    {
        ...
        // ========================================================================
        // 2025-11-19 修复内存泄漏: 限制队列大小 (GitHub Issue #155 官方方案)
        // 业务需求: 闭环检测需要历史图像进行匹配，但不需要全部历史数据
        // 队列管理策略: 保留最近60帧 (约4秒@15fps)，丢弃更旧的帧
        // 业务影响: 无负面影响 - 4秒时间窗口足够检测短期回环
        // 社区验证: HKUST-Aerial-Robotics/VINS-Fusion Issue #155 官方认证方案
        // ========================================================================
        const size_t MAX_LOOP_BUFFER = 60;  // 保留60帧历史 (官方推荐值)

        // 限制image_buf队列大小 (闭环检测用的图像)
        while (image_buf.size() > MAX_LOOP_BUFFER) {
            image_buf.pop();  // 丢弃最旧的图像，保留最新的
            // 仅在队列显著超限时警告，避免刷屏
            if (image_buf.size() % 20 == 0) {
                RCLCPP_WARN(rclcpp::get_logger("pose_graph"),
                            "loop_fusion image_buf限制生效: 保留最近%zu帧 (当前队列%zu帧)",
                            MAX_LOOP_BUFFER, image_buf.size());
            }
        }

        // 限制point_buf队列大小 (特征点云，与图像对应)
        while (point_buf.size() > MAX_LOOP_BUFFER) {
            point_buf.pop();
        }

        // 限制pose_buf队列大小 (位姿数据，与图像对应)
        while (pose_buf.size() > MAX_LOOP_BUFFER) {
            pose_buf.pop();
        }
        // ========================================================================
        // 队列大小限制结束，以下是原有的时间戳同步逻辑 (完全不变)
        // ========================================================================
